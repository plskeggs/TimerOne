...
ISR(TIMER1_OVF_vect)                                                           // interrupt service routine that wraps a user defined function supplied by attachInterrupt
{
   if (Timer1.isrCallback)
      Timer1.isrCallback(true);
   else if (Timer1.enableSwPwm)                                                // if we're in software pwm, let user handle PIO if they set a callback, otherwise we do it here
      digitalWrite(Timer1.outputPin, 1);                                       // warning: this takes about 25us at 8MHz, so setPeriod() should be called with a sufficiently large value that you don't
                                                                               // notice how coarse the least significant time interval is
}
...
ISR(TIMER1_COMPA_vect)
{
   ...
   {
      if (Timer1.isrCallback)                                                  // only for software pwm, do another callback on the falling edge
         Timer1.isrCallback(false);
      else
...
void TimerOne::attachInterrupt(void (*isr)(bool overflow), long microseconds)
{
   if (microseconds > 0) setPeriod(microseconds);
